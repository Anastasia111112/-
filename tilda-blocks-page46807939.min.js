...
t668__animateScroll()}
var t605_BTN_MORE_TRANSLATIONS={EN:'Read more',RU:'Читать полностью',FR:'Lire la suite',DE:'Mehr lesen',ES:'Leer más',PT:'Leia mais',JA:'もっと読む',ZH:'阅读更多',UK:'Читати далі',PL:'Czytaj więcej',KK:'Көбірек оқу',IT:'Leggi di più',LV:'Lasīt vairāk',};function t605_initClampTextEditorMode(recid){var rec=document.getElementById('rec'+recid);if(!rec||!window.MutationObserver)return;var texts=rec.querySelectorAll('.t605__text');texts.forEach(function(textEl){new MutationObserver(function(){t605_updateSlider(recid)}).observe(textEl,{childList:!0})})}
function t605_initClampText(recid){var rec=document.getElementById('rec'+recid);if(!rec)return;var texts=rec.querySelectorAll('.t605__text');texts.forEach(function(textEl){t605_setClampTextElement(textEl,recid)})}
function t605_setClampTextElement(textEl,recid){if(!textEl||!window.ResizeObserver)return;var isExpanded=!1;var btnMore=createShowMoreBtn();var innerDiv=document.createElement('div');innerDiv.innerHTML=textEl.innerHTML;textEl.innerHTML='';textEl.append(innerDiv);textEl.append(btnMore);clampText();handleClamp();new ResizeObserver(function(){if(isExpanded)return;handleClamp()}).observe(textEl);function handleClamp(){var isClamped=isTextClamped();if(isClamped){showMoreBtn()}else{hideMoreBtn()}
t605_updateSlider(recid)}
function isTextClamped(){return innerDiv.scrollHeight>innerDiv.clientHeight}
function clampText(){innerDiv.classList.add('t605__text-clamp')}
function expandText(){innerDiv.classList.remove('t605__text-clamp')}
function showMoreBtn(){btnMore.classList.remove('t605__btn-more_hide')}
function hideMoreBtn(){btnMore.classList.add('t605__btn-more_hide')}
function createShowMoreBtn(){var btn=document.createElement('a');var lang=getLang();btn.innerText=t605_BTN_MORE_TRANSLATIONS[lang]||t605_BTN_MORE_TRANSLATIONS.EN;btn.setAttribute('href','#');btn.setAttribute('role','button');btn.setAttribute('aria-label','Show more');btn.classList.add('t605__btn-more');btn.addEventListener('click',function(event){event.preventDefault();btn.remove();expandText();isExpanded=!0;t605_updateSlider(recid)});return btn}
function getLang(){var lang='';var allrecords=document.querySelector('#allrecords');if(allrecords){lang=allrecords.getAttribute('data-tilda-project-lang')}
if(!lang){lang=(window.navigator.userLanguage||window.navigator.language).slice(0,2)}
return lang?lang.toUpperCase():'EN'}}
function t605_updateSlider(recid){t_onFuncLoad('t_slds_SliderHeight',function(){t_slds_SliderHeight(recid)})}
